...
class Speaker:
    def __init__(self, name, gpt, sovits, phones=None, bert=None, prompt=None):
        ...
        self.bert = bert
        self.prompt = prompt
        # Cache reference features for the active speaker; model switches replace this object.
        self.reference_cache_key = None
...
_reference_cache_enabled = os.environ.get("OWVOICE_REFERENCE_CACHE", "1").strip().lower() not in {
    "0",
    "false",
    "off",
}
...
def cut_text(text, punc):
    allowed = {",", ".", ";", "?", "!", "、", "，", "。", "？", "！", "；", "：", "…"}
    punc_set = {char for char in punc if char in allowed}
    if punc_set:
        text = text.strip("\n")
        chunks = []
        current = []
        for index, char in enumerate(text):
            ...
            is_last_in_group = (
                char in punc_set
                and (index + 1 == len(text) or text[index + 1] not in punc_set)
            )
            if is_last_in_group:
...
def _reference_signature(ref_wav_path, prompt_text, prompt_language, version, spk):
    """Return a cache key that changes when the reference input or model context changes."""
    try:
        stat = os.stat(ref_wav_path)
    except OSError:
        return None
    return (
        os.path.abspath(ref_wav_path),
        stat.st_size,
        stat.st_mtime_ns,
        prompt_text.strip(),
        prompt_language,
...
def _prepare_reference_features(
    ...
    """Prepare reference features and reuse them for repeated requests of one active model."""
    speaker = speaker_list[spk]
    cacheable = _reference_cache_enabled and not inp_refs
    cache_key = _reference_signature(ref_wav_path, prompt_text, prompt_language, version, spk)

    with _reference_cache_lock:
        if cacheable and cache_key is not None and speaker.reference_cache_key == cache_key:
            return speaker.reference_cache, True
        ...
        with torch.no_grad():
            ...
            codes = speaker_list[spk].sovits.vq_model.extract_latent(ssl_content)
            ...
            if version not in {"v3", "v4"}:
                ...
                if inp_refs:
                    for path in inp_refs:
                        try:
                            refer_item, audio_tensor = get_spepc(
                                speaker_list[spk].sovits.hps, path.name, dtype, device, is_v2pro
                            )
                            refers.append(refer_item)
                            if is_v2pro:
                ...
                if len(refers) == 0:
                    refer_item, audio_tensor = get_spepc(
                        speaker_list[spk].sovits.hps, ref_wav_path, dtype, device, is_v2pro
                    )
                    refers = [refer_item]
                    if is_v2pro:
                ...
                refer, audio_tensor = get_spepc(
                    speaker_list[spk].sovits.hps, ref_wav_path, dtype, device
                )

        result = {"prompt": prompt, "refer": refer, "refers": refers, "sv_emb": sv_emb}
        if cacheable and cache_key is not None:
            speaker.reference_cache_key = cache_key
            speaker.reference_cache = result
        return result, False
...
def get_tts_wav(
    ...
    reference_features, reference_cache_hit = _prepare_reference_features(
        ref_wav_path,
        prompt_text,
        prompt_language,
        version,
    ...
    prompt = reference_features["prompt"]
    refer = reference_features["refer"]
    refers = reference_features["refers"]
    sv_emb = reference_features["sv_emb"]
    is_v2pro = version in {"v2Pro", "v2ProPlus"}
    t1 = ttime()
    ...
    for text in texts:
        ...
        if _performance_diagnostics:
            logger.info(
                "[PERF] segment chars=%d ref_cache=%s reference=%.3fs text_prepare=%.3fs semantic=%.3fs decode=%.3fs total=%.3fs",
                len(text),
                reference_cache_hit,
                t1 - t0,
                t2 - t1,
    ...
    if _performance_diagnostics:
        logger.info("[PERF] request complete total=%.3fs ref_cache=%s", ttime() - t0, reference_cache_hit)
