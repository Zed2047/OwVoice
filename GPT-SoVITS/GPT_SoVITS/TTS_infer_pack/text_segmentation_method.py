...
def split(todo_text):
    ...
    todo_texts = []
    current = []
    for index, char in enumerate(todo_text):
        current.append(char)
        if char in splits and (index + 1 == len(todo_text) or todo_text[index + 1] not in splits):
            todo_texts.append("".join(current))
            current = []
    if current:
        todo_texts.append("".join(current))
    return todo_texts
    ...
    for i, char in enumerate(inp):
        if char in punds:
            if char == "." and i > 0 and i < len(inp) - 1 and inp[i - 1].isdigit() and inp[i + 1].isdigit():
                ...
                items.append(char)
                if i + 1 == len(inp) or inp[i + 1] not in punds:
